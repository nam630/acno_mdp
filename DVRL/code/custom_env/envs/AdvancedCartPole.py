...
'''
NOTE (1/28)
- exps from before 1/28 didn't enforce 100 trajectory length consistently across all environments
- it allowed the cartpole to run as long as it can before accumulating 100 steps in the successful zone but there's no penalty for staying in the neutral/unsuccessful zone & "failing" only incurred -1500
- also run with the option of a more fine-grained reward setting (where the agent not only has to stay in the neutral zone but is rewarded for being successful)
'''
class CustomCartPoleEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.cost = obs_cost
        self.max_episode_step = 200 # you always end at step 100
        self.final_rew = 0
        self.obs_num = 0
        self.total_rew = 0
    ...
    def step(self, combined_action):
        ...
        return np.array(self.obs).copy(), reward, done, {'action': action, 'success': self.steps==self.max_episode_step, 'comb action': combined_action, 'observed': observe, 'true state': np.array(self.state), 'fell': fell, 'obs_num':self.obs_num, 'steps': self.steps, 'total rew': self.total_rew}
