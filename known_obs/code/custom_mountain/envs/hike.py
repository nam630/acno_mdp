...
class CustomHikeEnv(gym.Env):
    ...
    def __init__(self,
                 ...
                 shaping_power=4,
                 hill_height=4):
        ''' additional features for logging '''
        self.total_rew = 0.
        self.obs_num = 0
        self.steps = 0
        self.obs_pos = [] # track observed pos
        self.no_pos = [] # track unobserved pos
    ...
    def _step(self, _action):
        ...
        if self.steps == 75:
            ...
            fig.savefig('obs_1.png')
            assert(False)
        info = {
            'success': self.reached_goal(),
            'total rew': self.total_rew,
    ...
    def _render(self, mode='human', close=False):
        ...
        if mode == 'rgb_array':
            ...
            #             antialiased=True)
            x = np.array(self.no_pos)[:,0] * 10.
            y = np.array(self.no_pos)[:,1] * 10.
            print(x, y)
            ax.scatter(x, y, marker='o', s=5, c='black')
            x = np.array(self.obs_pos)[:,0] * 10.
            y = np.array(self.obs_pos)[:,1] * 10.
            print(x, y)
