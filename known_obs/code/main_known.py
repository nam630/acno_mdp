...
def general_config(cuda, algorithm, environment, rl_setting, loss_function, log):
    ...
    for key3 in environment['particle_filter_adaptation']:
        algorithm['particle_filter'][key3] = environment['particle_filter_adaptation'][key3]
...
def setup(rl_setting, device, _run, _log, log, seed, cuda):
    ...
    # this first action is always assumed to be observing
...
def create_model(envs, algorithm, rl_setting):
    ...
    from known_pf_model_alg2 import DVRLPolicy

    if algorithm['use_particle_filter']:
        model_params = algorithm['model']
        model_params.update(algorithm['particle_filter'])
        model_params['env_type'] = algorithm['env_type']
        model_params['savedir'] = algorithm['dirname']
        # if not os.path.exists(model_params['savedir']):
...
def run_model(actor_critic, current_memory, envs,
              ...
              predicted_times=None, verbose=True, savedir=None):
    """
    Runs the model.
    ...
    """
    batch_sz = rl_setting['num_processes']
    # Run model
    ...
    return policy_return, current_memory, blank_mask, masks, reward, info
...
def track_values(tracked_values, policy_return):
    ...
    Args:
        ...
        old_observation
    """
    # value and action log probs are also updated with h(obs)
...
def main(_run,
    ...
    for j in range(num_updates):
        ...
        for step in range(rl_setting['num_steps']):
            policy_return, current_memory, blank_mask, masks, reward, _info = run_model(
